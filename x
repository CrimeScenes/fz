...
local function isPlayerHealthCheck(player)
    local humanoid = player.Character and player.Character:FindFirstChild("Humanoid")
    return humanoid and humanoid.Health >= 100 -- Only shoot if health is 100 or more
end

local function isWallBetween(playerPosition, targetPosition)
    local ray = Ray.new(playerPosition, (targetPosition - playerPosition).Unit * (playerPosition - targetPosition).Magnitude)
...
local function TriggerBotAction()
    ...
    for _, player in ipairs(Players:GetPlayers()) do
        if player ~= LocalPlayer and player.Character then
            if isPlayerAlive(player) and isPlayerHealthCheck(player) then
                local closestPart = findClosestPart(player.Character)
                if closestPart then
                    local predictedPosition = getPredictedPosition(player.Character)
                    if predictedPosition and isWithinBoxFOV(predictedPosition) and not isWallBetween(LocalPlayer.Character.PrimaryPart.Position, closestPart.Position) then
...
-- // Update FOV Function // --
local function updateFov()
    local settings = getgenv().Silent.Normal.FovSettings
    clearFovParts()
...
local function isPartInFovAndVisible(part)
    ...
    return onScreen and distance <= getgenv().Silent.Normal.FovSettings.FovRadius
end


-- // Check if Part Visible // --
local function isPartVisible(part)
    if not getgenv().Silent.Normal.WallCheck then 
        return true
    end
...
local OldPredictionY = getgenv().Silent.Normal.Prediction
local function GetVelocity(player, part)
    if player and player.Character then
        local velocity = player.Character[part].Velocity
        if velocity.Y < -30 and getgenv().Silent.Normal.Resolver then
            getgenv().Silent.Normal.Prediction = 0
            return velocity
        elseif velocity.Magnitude > 50 and getgenv().Silent.Normal.Resolver then
            return player.Character:FindFirstChild("Humanoid").MoveDirection * 16
        else
            getgenv().Silent.Normal.Prediction = OldPredictionY
            return velocity
        end
    end
...
local function toggleFeature()
    getgenv().Silent.Normal.Enabled = not getgenv().Silent.Normal.Enabled
    local status = getgenv().Silent.Normal.Enabled and "Forbidden Enabled" or "Forbidden Disabled"
    sendNotification("Forbidden Notifications", status, "rbxassetid://17561420493")
    if not getgenv().Silent.Normal.Enabled then
        Fov.Visible = false
        clearFovParts()
    end
end
...
Run_Service.RenderStepped:Connect(function()
    if getgenv().Silent.Normal.Enabled and IsTargeting then  -- Only work when CamLock is engaged
        ...
        if Target and Target.Character then
            ...
            if closestPart and closestPoint then
                ...
                Replicated_Storage[RemoteEvent]:FireServer(Argument, closestPoint + velocity * getgenv().Silent.Normal.Prediction)
            end
        end
    else
...
task.spawn(function()
    while task.wait(0.1) do
        if getgenv().Silent.Normal.Enabled then
            Target = GetClosestPlr()
            Fov.Visible = IsTargeting and getgenv().Silent.Normal.FovSettings.FovVisible  -- Update visibility based on targeting
        end
    end
end)
...
if getgenv().Adjustment.Checks.NoGroundShots == true then
    local function CheckNoGroundShots(Plr)
        if getgenv().Adjustment.Checks.NoGroundShots and Plr.Character:FindFirstChild("Humanoid") and Plr.Character.Humanoid:GetState() == Enum.HumanoidStateType.Freefall then
            pcall(function()
                local TargetVelv5 = Plr.Character:FindFirstChild(getgenv().Silent.Normal and getgenv().Silent.Normal)
                if TargetVelv5 then
